...
export default function Index({ products, categories, filters }) {
    ...
    const metaDescription = filters.search
        ? t('Résultats pour "{{search}}" — Compléments alimentaires et produits de nutrition à Marrakech.', { search: filters.search })
        : t('Découvrez notre catalogue de produits de nutrition, compléments alimentaires et suppléments sportifs à Marrakech. Livraison à domicile.');

    return (
        <Layout>
            <Head title={t('Catalogue produits')}>
                <meta head-key="description" name="description" content={metaDescription} />
                <meta head-key="robots" name="robots" content="index, follow" />
                <link head-key="canonical" rel="canonical" href={canonicalUrl} />
                <meta head-key="og:title" property="og:title" content={t('Catalogue produits — MarrakechNutrition')} />
            ...
            <div className="max-w-7xl mx-auto py-8 px-4">
                ...
                <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    <h1 className="text-2xl font-bold">{t('Produits')}</h1>
                ...
                {products.data.length === 0 ? (
                    <p className="text-gray-600 py-8 text-center">{t('Aucun produit trouvé.')}</p>
                ) : (
                ...
                {products.last_page > 1 && (
                    <nav aria-label={t('Pagination')} className="mt-6 flex gap-1">
